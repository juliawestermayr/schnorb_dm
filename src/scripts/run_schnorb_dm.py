...
def train(args, model, train_loader, val_loader, device):
    ...
    metrics = [MeanAbsoluteError(SchNOrbProperties.dm_prop),
    ...
    if args.logger == 'csv':
        ...
        metrics.append(HeatmapMAE(SchNOrbProperties.dm_prop))
...
def predict_dataset(model, loader, device, limit=None):
    properties = [SchNOrbProperties.dm_prop]
    ...
    for batch in tqdm(loader, ncols=100):
        ...
        Href = batch[SchNOrbProperties.dm_prop].cpu().detach().numpy()
        Hpred = result[0].cpu().detach().numpy()

        results[SchNOrbProperties.ham_prop].append(Hpred)

        reference[SchNOrbProperties.dm_prop].append(Href)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        mean, stddev = torch.Tensor(1),torch.Tensor(0) # stddev[SchNOrbProperties.en_prop]
    else:
